...
for (const vendor of vendors) {
  for (const lang of languages) {
    ...
    if (vendor.kind === 'store02') for (const required of ['data-menu-open','data-menu-dialog','foodpanda.com.tw/restaurant/ifkl/','ubereats.com/tw/store/','W22nek-QU_6DFcnkQXLMfg','rel="noopener sponsored"','class="vendor-gallery"','store-02-yakisoba-bread.webp','store-02-braised-pork-rice.webp','store-02-menu-overview.webp','store-02-shop-interior.webp','store-02-shiba-inu.webp','store-02-dining-room.webp','store-02-exterior-sign.webp','thai-green-tea.webp','tel:+886975041213','0975 041 213','facebook.com/share/1EL6oevq2E/','"telephone":"+886-975-041-213"','"sameAs":["https://www.facebook.com/share/1EL6oevq2E/"]']) if (!html.includes(required)) errors.push(`${page}: missing ${required}`);
    if (vendor.kind === 'store02') {
      for (const forbidden of ['maps.app.goo.gl']) if (html.includes(forbidden)) errors.push(`${page}: obsolete map data remains: ${forbidden}`);
